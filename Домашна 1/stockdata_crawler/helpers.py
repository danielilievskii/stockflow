...
def isoformat_to_eu_format(date_str):
    dt = datetime.fromisoformat(date_str)
    return dt.strftime('%d.%m.%Y')

def eu_format_to_isoformat(custom_date_str):
...
def us_to_iso_format(date_str):
    date_obj = datetime.strptime(date_str, '%m/%d/%Y')
    return date_obj.strftime('%Y-%m-%d')

def us_to_datetime(date_str):
...
def transform_commas_and_dots(input_str, to_dot=True):
...
def transform_commas_and_append_00(value):
