...
from app.crawler_news.crawler import fetch_company_news_data
from app.crawler_news.config import BASE_URL, COMPANIES_URL, HEADERS
from app.crawler_news.data_utils import save_latest_dates_to_db, load_latest_dates_from_db, append_data_to_db
...
async def filter_1_fetch_companies():
    ...
    print("Filter 1 - News Crawler: Fetched companies.")
    return companies
...
async def filter_2_initialize_dates(companies):
    ...
    if not company_latest_dates:
        print("Filter 2 - News Crawler: No latest dates found in DB. Initializing with None.")
        company_latest_dates = {company: None for company in companies}
    else:
        print("Filter 2 - News Crawler: Loaded latest dates from DB.")
...
def filter_3_fetch_missing_data(company_latest_dates):
    ...
    with Manager() as manager:
        ...
        #save_latest_dates_to_db(all_latest_dates)
        print("Filter 3 - News Crawler: Completed fetching and updating news data.")
